UPDATE sigas_report
SET jrxml=cast('<!-- Created with Jaspersoft Studio version 6.2.1.final using JasperReports Library version 6.2.1  -->
<!-- 2020-05-15T12:07:38 -->
<jasperReport xmlns="http://jasperreports.sourceforge.net/jasperreports" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://jasperreports.sourceforge.net/jasperreports http://jasperreports.sourceforge.net/xsd/jasperreport.xsd" name="dichiarazione" pageWidth="1000" pageHeight="850" columnWidth="980" leftMargin="20" rightMargin="0" topMargin="20" bottomMargin="0" whenResourceMissingType="Empty" isIgnorePagination="true" uuid="8b824d48-44af-42b8-864e-1eac359ac32f">
	...
	<subDataset name="dichiarazioneVersamenti" uuid="878aa02e-f5d0-46db-95ab-ebf65634f7ea">
		...
		<queryString>
			<![CDATA[select annualita, mese, v.importo_complessivo ,
						(select p.sigla_provincia from sigas_provincia p where p.id_provincia = v.fk_provincia ) as provincia,
						importo, note, 
						...
						(select totale_dich from sigas_dich_consumi 
							where id_anag =  $P{id_anag} and annualita = $P{anno} 
								and id_consumi = v.id_consumi 
								and (CASE when $P{id_provincia} <> 0 then provincia_erogazione = (select p.sigla_provincia from sigas_provincia p where p.id_provincia = $P{id_provincia}) else true end)),
						(select conguaglio_calcolato from sigas_dich_consumi 
							where id_anag =  $P{id_anag} and annualita = $P{anno} 
							and id_consumi = v.id_consumi 
							and (CASE when $P{id_provincia} <> 0 then provincia_erogazione = (select p.sigla_provincia from sigas_provincia p where p.id_provincia = $P{id_provincia}) else true end)),
						(select rateo_dich from sigas_dich_consumi 
							where id_anag =  $P{id_anag} and annualita = $P{anno} 
							and id_consumi = v.id_consumi 
							and (CASE when $P{id_provincia} <> 0 then provincia_erogazione = (select p.sigla_provincia from sigas_provincia p where p.id_provincia = $P{id_provincia}) else true end)),
						(select (CASE WHEN v.mese = ''Gennaio'' THEN 1
								 WHEN v.mese = ''Febbraio'' THEN 2
								 WHEN v.mese = ''Marzo'' THEN 3
								 WHEN v.mese = ''Aprile'' THEN 4
								 WHEN v.mese = ''Maggio'' THEN 5
								 WHEN v.mese = ''Giugno'' THEN 6
								 WHEN v.mese = ''Luglio'' THEN 7
								 WHEN v.mese = ''Agosto'' THEN 8
								 WHEN v.mese = ''Settembre'' THEN 9
								 WHEN v.mese = ''Ottobre'' THEN 10
								 WHEN v.mese = ''Novembre'' THEN 11
								 WHEN v.mese = ''Dicembre'' THEN 12
									END  )*rateo_dich+conguaglio_dich+v.importo-(select importi from (SELECT mese, id_tipo_versamento, id_anag, fk_provincia ,sum(importo)
  OVER (ORDER BY mese ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) importi
FROM sigas_dich_versamenti sdv 
where annualita = $P{anno} and id_anag = $P{id_anag}
						and (CASE when $P{id_provincia} <> 0 then fk_provincia = $P{id_provincia} else true end)
						and (CASE when $P{mese}  is not null then mese = $P{mese} else true end) 
					  	and (CASE when $P{id_tipo_versamento} <> 0 then id_tipo_versamento = $P{id_tipo_versamento} else true end)
					  	and id_tipo_versamento !=4) as importicalcolati where mese=v.mese and id_tipo_versamento = v.id_tipo_versamento and id_anag = v.id_anag and fk_provincia =v.fk_provincia )
					  	from sigas_dich_consumi 
							where id_anag =  $P{id_anag} and annualita = $P{anno} 
							and id_consumi = v.id_consumi 
							and (CASE when $P{id_provincia} <> 0 then provincia_erogazione = (select p.sigla_provincia from sigas_provincia p where p.id_provincia = $P{id_provincia}) else true end)) as importo_calcolato,
							(select (CASE WHEN v.mese = ''Gennaio'' THEN 1
								 WHEN v.mese = ''Febbraio'' THEN 2
								 WHEN v.mese = ''Marzo'' THEN 3
								 WHEN v.mese = ''Aprile'' THEN 4
								 ...
								 WHEN v.mese = ''Novembre'' THEN 11
								 WHEN v.mese = ''Dicembre'' THEN 12
									END  )*rateo_dich+conguaglio_dich-(select importi from (SELECT mese,  id_tipo_versamento, id_anag, fk_provincia,  sum(importo)
  OVER (ORDER BY mese ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) importi
FROM sigas_dich_versamenti sdv 
where annualita = $P{anno} and id_anag = $P{id_anag}
						and (CASE when $P{id_provincia} <> 0 then fk_provincia = $P{id_provincia} else true end)
						and (CASE when $P{mese} is not  null then mese = $P{mese} else true end) 
					  	...
					  	from sigas_dich_consumi 
							where id_anag =  $P{id_anag} and annualita = $P{anno} 
							and id_consumi = v.id_consumi 
							and (CASE when $P{id_provincia} <> 0 then provincia_erogazione = (select p.sigla_provincia from sigas_provincia p where p.id_provincia = $P{id_provincia}) else true end) ) as differenza
					from sigas_dich_versamenti v
					where annualita = $P{anno} and id_anag = $P{id_anag}
						and (CASE when $P{id_provincia} <> 0 then fk_provincia = $P{id_provincia} else true end)
						and (CASE when $P{mese} is not null then mese = $P{mese} else true end) 
					  	and (CASE when $P{id_tipo_versamento} <> 0 then id_tipo_versamento = $P{id_tipo_versamento} else true end)
					 order by (CASE WHEN v.mese = ''Gennaio'' THEN 1
								 WHEN v.mese = ''Febbraio'' THEN 2
								 WHEN v.mese = ''Marzo'' THEN 3
								 WHEN v.mese = ''Aprile'' THEN 4
								 WHEN v.mese = ''Maggio'' THEN 5
								 ...
								 WHEN v.mese = ''Novembre'' THEN 11
								 WHEN v.mese = ''Dicembre'' THEN 12
									END  ) asc, tipo desc, fk_provincia asc;]]>
		</queryString>
		...
		<field name="importo_calcolato" class="java.math.BigDecimal"/>
		<field name="differenza" class="java.math.BigDecimal"/>
	...
	<parameter name="id_tipo_versamento" class="java.lang.Long"/>
	<parameter name="mese" class="java.lang.String"/>
	<parameter name="provinciaFiltro" class="java.lang.String"/>
	<parameter name="tipologiaFiltro" class="java.lang.String"/>
	<queryString>
		...
		<band height="95" splitType="Stretch">
			...
			<textField>
				<reportElement x="0" y="31" width="240" height="30" isRemoveLineWhenBlank="true" uuid="1b954333-b28f-461d-8002-e91568bb4fee">
					<printWhenExpression><![CDATA[$P{provinciaFiltro} != null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Provincia:  " + $P{provinciaFiltro}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="358" y="31" width="622" height="30" isRemoveLineWhenBlank="true" uuid="82559a4e-db38-426d-b17e-007b35a7fef9">
					<printWhenExpression><![CDATA[$P{tipologiaFiltro} != null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Tipologia:  " + $P{tipologiaFiltro}]]></textFieldExpression>
			</textField>
		</band>
		<band height="76">
			<property name="com.jaspersoft.studio.unit.height" value="pixel"/>
					...
					<jr:column width="120" uuid="f30c151f-33ea-4f93-be7c-c485265b3bbe">
						...
						<jr:detailCell style="Table 3_TD" height="30">
							<textField pattern="#,##0.00 €" isBlankWhenNull="true">
								...
								<textFieldExpression><![CDATA[($F{importo_calcolato})]]></textFieldExpression>
							</textField>
						</jr:detailCell>
					</jr:column>
					<jr:column width="120" uuid="99c8d113-aaa7-4391-8954-6727a83fdc84">
						...
						<jr:detailCell style="Table 3_TD" height="30">
							<textField pattern="#,##0.00 €" isBlankWhenNull="true">
								...
								<textFieldExpression><![CDATA[$F{differenza}]]></textFieldExpression>
							</textField>
						</jr:detailCell>
					</jr:column>
