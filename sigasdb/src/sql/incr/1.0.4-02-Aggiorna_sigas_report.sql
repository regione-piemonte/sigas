UPDATE sigas_report
SET jrxml=cast('<?xml version="1.0" encoding="UTF-8"?>
<!-- Created with Jaspersoft Studio version 6.2.1.final using JasperReports Library version 6.2.1  -->
<!-- 2020-06-18T10:05:01 -->
<jasperReport xmlns="http://jasperreports.sourceforge.net/jasperreports" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://jasperreports.sourceforge.net/jasperreports http://jasperreports.sourceforge.net/xsd/jasperreport.xsd" name="dichiarazione" pageWidth="1000" pageHeight="850" columnWidth="980" leftMargin="20" rightMargin="0" topMargin="20" bottomMargin="0" whenResourceMissingType="Empty" isIgnorePagination="true" uuid="8b824d48-44af-42b8-864e-1eac359ac32f">
	<property name="com.jaspersoft.studio.data.sql.tables" value=""/>
	<property name="net.sf.jasperreports.export.xls.white.page.background" value="false"/>
	<property name="net.sf.jasperreports.export.xls.detect.cell.type" value="true"/>
	<property name="net.sf.jasperreports.export.xls.ignore.graphics" value="false"/>
	<property name="net.sf.jasperreports.exports.xls.font.size.fix.enabled" value="false"/>
	<property name="net.sf.jasperreports.export.xls.remove.empty.space.between.columns" value="false"/>
	<property name="net.sf.jasperreports.export.xls.remove.empty.space.between.rows" value="false"/>
	<property name="com.jaspersoft.studio.data.defaultdataadapter" value="Sample DB"/>
	<property name="com.jaspersoft.studio.unit." value="pixel"/>
	<property name="com.jaspersoft.studio.unit.pageHeight" value="pixel"/>
	<property name="com.jaspersoft.studio.unit.pageWidth" value="pixel"/>
	<property name="com.jaspersoft.studio.unit.topMargin" value="pixel"/>
	<property name="com.jaspersoft.studio.unit.bottomMargin" value="pixel"/>
	<property name="com.jaspersoft.studio.unit.leftMargin" value="pixel"/>
	<property name="com.jaspersoft.studio.unit.rightMargin" value="pixel"/>
	<property name="com.jaspersoft.studio.unit.columnWidth" value="pixel"/>
	<property name="com.jaspersoft.studio.unit.columnSpacing" value="pixel"/>
	<style name="Table_CH" mode="Opaque" backcolor="#BFE1FF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 1_TH" mode="Opaque" backcolor="#F0F8FF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 1_CH" mode="Opaque" backcolor="#BFE1FF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 1_TD" mode="Opaque" backcolor="#FFFFFF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 2_TH" mode="Opaque" backcolor="#F0F8FF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 2_CH" mode="Opaque" backcolor="#BFE1FF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 2_TD" mode="Opaque" backcolor="#FFFFFF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 3_TH" mode="Opaque" backcolor="#F0F8FF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 3_CH" mode="Opaque" backcolor="#BFE1FF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<style name="Table 3_TD" mode="Opaque" backcolor="#FFFFFF">
		<box>
			<pen lineWidth="0.5" lineColor="#000000"/>
			<topPen lineWidth="0.5" lineColor="#000000"/>
			<leftPen lineWidth="0.5" lineColor="#000000"/>
			<bottomPen lineWidth="0.5" lineColor="#000000"/>
			<rightPen lineWidth="0.5" lineColor="#000000"/>
		</box>
	</style>
	<subDataset name="dichiarazioneVersamenti" uuid="878aa02e-f5d0-46db-95ab-ebf65634f7ea">
		<property name="com.jaspersoft.studio.data.sql.tables" value=""/>
		<property name="com.jaspersoft.studio.data.defaultdataadapter" value="Sample DB"/>
		<parameter name="anno" class="java.lang.String"/>
		<parameter name="id_anag" class="java.lang.Long"/>
		<parameter name="id_provincia" class="java.lang.Long"/>
		<parameter name="id_tipo_versamento" class="java.lang.Long"/>
		<parameter name="mese" class="java.lang.String"/>
		<queryString>
			<![CDATA[select
						provincia_erogazione,
						sdv.importo totale_versato,
						sdc.totale_dich totale_dichiarazione,
						sdc.addizionale_liquidata,
						0 as Priority 
					from
						sigas_dich_consumi sdc
					left join sigas_dich_versamenti sdv on
						sdc.id_anag = sdv.id_anag
						and sdc.annualita = sdv.annualita
						and sdc.provincia_erogazione = (
						select
							p.sigla_provincia
						from
							sigas_provincia p
						where
							p.id_provincia = sdv.fk_provincia)
					where
						sdc.id_anag = $P{id_anag}
						and sdc.annualita = $P{anno}
					union
					select
						''totale'',
						sum(sdv.importo) as totale_versato,
						sum(sdc.totale_dich) totale_dichiarazione,
						sum(sdc.addizionale_liquidata) addizionale_liquidata,
						1 as Priority 
					from
						sigas_dich_consumi sdc
					left join sigas_dich_versamenti sdv on
						sdc.id_anag = sdv.id_anag
						and sdc.annualita = sdv.annualita
						and sdc.provincia_erogazione = (
						select
							p.sigla_provincia
						from
							sigas_provincia p
						where
							p.id_provincia = sdv.fk_provincia)
					where
						sdc.id_anag = $P{id_anag}
						and sdc.annualita = $P{anno} 
					order by Priority asc, provincia_erogazione asc;]]>
		</queryString>
		<field name="provincia_erogazione" class="java.lang.String"/>
		<field name="totale_versato" class="java.math.BigDecimal"/>
		<field name="totale_dichiarazione" class="java.math.BigDecimal"/>
		<field name="addizionale_liquidata" class="java.math.BigDecimal"/>
	</subDataset>
	<parameter name="anno" class="java.lang.String">
		<parameterDescription><![CDATA[]]></parameterDescription>
	</parameter>
	<parameter name="id_anag" class="java.lang.Long"/>
	<parameter name="id_provincia" class="java.lang.Long"/>
	<parameter name="id_tipo_versamento" class="java.lang.Long"/>
	<parameter name="mese" class="java.lang.String"/>
	<queryString>
		<![CDATA[select denominazione,codice_azienda codiceAzienda, indirizzo, iban, telefono, pec, email, note, 
						(CASE when fideussione  then ''OK'' else ''KO'' end) fideussione , 
						importo_fideussione::money::numeric::float8||''€'' importo_fideussione, 
						(select sc.denom_comune from sigas_comune sc where id_comune = fk_comune) comune, 
					(select p.sigla_provincia from sigas_provincia p where p.id_provincia = fk_provincia) as provincia
				 from sigas_anagrafica_soggetti where id_anag = $P{id_anag}]]>
	</queryString>
	<field name="denominazione" class="java.lang.String"/>
	<field name="codiceAzienda" class="java.lang.String"/>
	<field name="indirizzo" class="java.lang.String"/>
	<field name="iban" class="java.lang.String"/>
	<field name="telefono" class="java.lang.String"/>
	<field name="pec" class="java.lang.String"/>
	<field name="email" class="java.lang.String"/>
	<field name="note" class="java.lang.String"/>
	<field name="fideussione" class="java.lang.String"/>
	<field name="importo_fideussione" class="java.lang.String"/>
	<field name="comune" class="java.lang.String"/>
	<field name="provincia" class="java.lang.String"/>
	<variable name="imgPiemonte" class="java.lang.String">
		<variableExpression><![CDATA[""]]></variableExpression>
	</variable>
	<columnHeader>
		<band height="280">
			<frame>
				<reportElement x="0" y="0" width="598" height="110" uuid="a5dc679a-1d12-4ebf-8584-785e75d6ff24"/>
				<image evaluationTime="Report">
					<reportElement x="0" y="0" width="360" height="60" uuid="35603c9e-801b-4bb5-8b11-5c2004529667"/>
					<imageExpression><![CDATA[javax.imageio.ImageIO.read(new java.io.ByteArrayInputStream(new sun.misc.BASE64Decoder().decodeBuffer($V{imgPiemonte})))]]></imageExpression>
				</image>
				<staticText>
					<reportElement x="0" y="60" width="360" height="15" uuid="bcc2eb00-281b-47be-9adb-6cd0068b6b65"/>
					<text><![CDATA[Direzione Risorse Finanziarie e Patrimonio]]></text>
				</staticText>
				<staticText>
					<reportElement x="0" y="75" width="360" height="15" uuid="48cec07b-685c-4ecf-934e-2c25bac72e72">
						<property name="com.jaspersoft.studio.unit.height" value="pixel"/>
					</reportElement>
					<text><![CDATA[Settore Politiche Fiscali e Contenzioso amministrativo]]></text>
				</staticText>
				<staticText>
					<reportElement x="0" y="90" width="360" height="20" uuid="ce4d62ee-aa2f-494a-9834-e0d4e7142d4e"/>
					<text><![CDATA[settore.tributi@regione.piemonte.it]]></text>
				</staticText>
			</frame>
			<staticText>
				<reportElement x="0" y="130" width="980" height="30" uuid="84c5a6e2-172f-45aa-8ed1-b15b80ea7275"/>
				<textElement textAlignment="Center" verticalAlignment="Middle">
					<font size="22"/>
				</textElement>
				<text><![CDATA[Sistema informativo Gas Naturale]]></text>
			</staticText>
			<staticText>
				<reportElement x="0" y="187" width="980" height="30" uuid="934eb491-17bc-4ef5-94c4-1a160dad06cd"/>
				<textElement textAlignment="Center" verticalAlignment="Middle">
					<font size="20"/>
				</textElement>
				<text><![CDATA[Dettaglio Soggetto]]></text>
			</staticText>
			<textField>
				<reportElement x="0" y="250" width="980" height="30" isRemoveLineWhenBlank="true" uuid="5a055f2e-38bc-4047-834e-c3a776614665">
					<printWhenExpression><![CDATA[$F{denominazione} != null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Denominazione:  " + $F{denominazione}]]></textFieldExpression>
			</textField>
		</band>
	</columnHeader>
	<detail>
		<band height="161" splitType="Stretch">
			<property name="com.jaspersoft.studio.unit.height" value="pixel"/>
			<textField>
				<reportElement x="0" y="1" width="340" height="30" isRemoveLineWhenBlank="true" uuid="120c2b25-5a21-4170-ad94-2739f07e1dbb">
					<printWhenExpression><![CDATA[$F{codiceAzienda} != null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Codice Azienda: " +$F{codiceAzienda}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="358" y="1" width="622" height="30" isRemoveLineWhenBlank="true" uuid="f2107440-952c-4f8c-9906-45048d569edf">
					<printWhenExpression><![CDATA[$F{indirizzo} != null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Indirizzo:  " + $F{indirizzo}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="0" y="31" width="340" height="30" isRemoveLineWhenBlank="true" uuid="1b954333-b28f-461d-8002-e91568bb4fee">
					<printWhenExpression><![CDATA[$F{comune}!= null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Comune:  " + $F{comune}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="358" y="31" width="622" height="30" isRemoveLineWhenBlank="true" uuid="82559a4e-db38-426d-b17e-007b35a7fef9">
					<printWhenExpression><![CDATA[$F{provincia}!= null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Provincia:  " + $F{provincia}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="1" y="61" width="339" height="30" isRemoveLineWhenBlank="true" uuid="c3b0281a-25c0-48ff-9e38-a4b7586cdbb7">
					<printWhenExpression><![CDATA[$F{iban}!= null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Iban: " +$F{iban}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="358" y="61" width="622" height="30" isRemoveLineWhenBlank="true" uuid="4cf908fc-8cee-45ba-8ce0-5a5f967769ca">
					<printWhenExpression><![CDATA[$F{telefono} != null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Telefono:  " + $F{telefono}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="0" y="91" width="340" height="30" isRemoveLineWhenBlank="true" uuid="64ab97f8-9fd0-4906-a201-fb313fed238e">
					<printWhenExpression><![CDATA[$F{pec}!= null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Pec:  " + $F{pec}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="358" y="91" width="622" height="30" isRemoveLineWhenBlank="true" uuid="82050338-03ae-4e12-8174-458a40cb4c56">
					<printWhenExpression><![CDATA[$F{email} != null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Email:  " + $F{email}]]></textFieldExpression>
			</textField>
			<textField>
				<reportElement x="0" y="121" width="580" height="30" isRemoveLineWhenBlank="true" uuid="f2da75e0-d8d4-4ae0-b663-6b2bac6996a5">
					<printWhenExpression><![CDATA[$F{fideussione}!= null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Copertura Fidejussoria:  " + $F{fideussione}+" ( "+$F{importo_fideussione}+") "]]></textFieldExpression>
			</textField>
		</band>
		<band height="121">
			<property name="com.jaspersoft.studio.unit.height" value="pixel"/>
			<componentElement>
				<reportElement key="" isPrintRepeatedValues="false" x="0" y="47" width="950" height="61" uuid="e15d6a63-1968-413d-91b5-53b9c4298d16">
					<property name="com.jaspersoft.studio.layout" value="com.jaspersoft.studio.editor.layout.VerticalRowLayout"/>
					<property name="com.jaspersoft.studio.table.style.table_header" value="Table 3_TH"/>
					<property name="com.jaspersoft.studio.table.style.column_header" value="Table 3_CH"/>
					<property name="com.jaspersoft.studio.table.style.detail" value="Table 3_TD"/>
					<property name="net.sf.jasperreports.export.headertoolbar.table.name" value="dichiarazioni"/>
				</reportElement>
				<jr:table xmlns:jr="http://jasperreports.sourceforge.net/jasperreports/components" xsi:schemaLocation="http://jasperreports.sourceforge.net/jasperreports/components http://jasperreports.sourceforge.net/xsd/components.xsd" whenNoDataType="AllSectionsNoDetail">
					<datasetRun subDataset="dichiarazioneVersamenti" uuid="2bf38382-8ac0-4f92-975a-7f5e90ec1e48">
						<datasetParameter name="anno">
							<datasetParameterExpression><![CDATA[$P{anno}]]></datasetParameterExpression>
						</datasetParameter>
						<datasetParameter name="id_anag">
							<datasetParameterExpression><![CDATA[$P{id_anag}]]></datasetParameterExpression>
						</datasetParameter>
						<datasetParameter name="id_provincia">
							<datasetParameterExpression><![CDATA[$P{id_provincia}]]></datasetParameterExpression>
						</datasetParameter>
						<datasetParameter name="id_tipo_versamento">
							<datasetParameterExpression><![CDATA[$P{id_tipo_versamento}]]></datasetParameterExpression>
						</datasetParameter>
						<datasetParameter name="mese">
							<datasetParameterExpression><![CDATA[$P{mese}]]></datasetParameterExpression>
						</datasetParameter>
						<connectionExpression><![CDATA[$P{REPORT_CONNECTION}]]></connectionExpression>
					</datasetRun>
					<jr:column width="100" uuid="ae322038-6425-4fb8-905c-a707074a6fa0">
						<property name="com.jaspersoft.studio.components.table.model.column.name" value="Colonna1"/>
						<jr:tableHeader style="Table 3_TH" height="30" rowSpan="1">
							<staticText>
								<reportElement x="0" y="0" width="100" height="30" uuid="6394592f-ee42-4224-a44c-c6850eb25077"/>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<text><![CDATA[Provincia]]></text>
							</staticText>
						</jr:tableHeader>
						<jr:detailCell style="Table 3_TD" height="30">
							<textField pattern="" isBlankWhenNull="true">
								<reportElement positionType="Float" x="0" y="0" width="100" height="30" uuid="1ce92a17-6806-43bc-bdf7-d0f7737cf10b"/>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<textFieldExpression><![CDATA[$F{provincia_erogazione}]]></textFieldExpression>
							</textField>
						</jr:detailCell>
					</jr:column>
					<jr:column width="140" uuid="5d6bf5be-cf92-4ae9-ae01-a215ebd1d7e4">
						<property name="com.jaspersoft.studio.components.table.model.column.name" value="Colonna2"/>
						<property name="com.jaspersoft.studio.unit.width" value="pixel"/>
						<jr:tableHeader style="Table 3_TH" height="30" rowSpan="1">
							<property name="com.jaspersoft.studio.unit.width" value="px"/>
							<staticText>
								<reportElement x="0" y="0" width="140" height="30" uuid="52399b0e-ede0-4756-81ee-6995b6817b0c"/>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<text><![CDATA[Totale Dichiarazione]]></text>
							</staticText>
						</jr:tableHeader>
						<jr:detailCell style="Table 3_TD" height="30">
							<textField pattern="#,##0.00 €" isBlankWhenNull="true">
								<reportElement positionType="Float" x="0" y="0" width="140" height="30" uuid="f94df620-e3c6-43a8-a467-f667749aaf9b"/>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<textFieldExpression><![CDATA[$F{totale_dichiarazione}]]></textFieldExpression>
							</textField>
						</jr:detailCell>
					</jr:column>
					<jr:column width="150" uuid="49746f88-33e7-40a7-9c8d-69a9e0fb6038">
						<property name="com.jaspersoft.studio.components.table.model.column.name" value="Colonna3"/>
						<property name="com.jaspersoft.studio.unit.width" value="pixel"/>
						<jr:tableHeader style="Table 3_TH" height="30" rowSpan="1">
							<property name="com.jaspersoft.studio.unit.width" value="px"/>
							<staticText>
								<reportElement positionType="Float" stretchType="RelativeToTallestObject" x="0" y="0" width="150" height="30" uuid="0a8f65e8-58ce-491a-8b1a-e2f012bdfab5">
									<property name="com.jaspersoft.studio.unit.width" value="pixel"/>
								</reportElement>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<text><![CDATA[Addizionale Liquidata]]></text>
							</staticText>
						</jr:tableHeader>
						<jr:detailCell style="Table 3_TD" height="30">
							<textField pattern="#,##0.00 €" isBlankWhenNull="true">
								<reportElement x="0" y="0" width="150" height="30" uuid="f6c4633e-53cf-4f53-8ef1-6813d4239313"/>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<textFieldExpression><![CDATA[$F{addizionale_liquidata}]]></textFieldExpression>
							</textField>
						</jr:detailCell>
					</jr:column>
					<jr:column width="123" uuid="9ae99573-011d-45cc-82fa-0691ab4bb66e">
						<property name="com.jaspersoft.studio.components.table.model.column.name" value="Colonna4"/>
						<jr:tableHeader style="Table 3_TH" height="30" rowSpan="1">
							<property name="com.jaspersoft.studio.unit.width" value="px"/>
							<staticText>
								<reportElement x="0" y="0" width="123" height="30" uuid="a2c5feef-315a-4fbc-b943-1f1f3ffab59b"/>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<text><![CDATA[Totale Versato]]></text>
							</staticText>
						</jr:tableHeader>
						<jr:detailCell style="Table 3_TD" height="30">
							<textField pattern="#,##0.00 €" isBlankWhenNull="true">
								<reportElement x="0" y="0" width="123" height="30" uuid="1046e737-fcc3-4c57-9f5c-fddaa2d8415e"/>
								<textElement textAlignment="Center" verticalAlignment="Middle">
									<font fontName="SansSerif" size="11"/>
								</textElement>
								<textFieldExpression><![CDATA[$F{totale_versato}]]></textFieldExpression>
							</textField>
						</jr:detailCell>
					</jr:column>
				</jr:table>
			</componentElement>
			<textField>
				<reportElement x="0" y="-10" width="980" height="30" isRemoveLineWhenBlank="true" uuid="738b3ea6-c191-4c61-8f0a-56ad83e18e10">
					<printWhenExpression><![CDATA[$F{note}!= null]]></printWhenExpression>
				</reportElement>
				<textElement textAlignment="Left" verticalAlignment="Middle">
					<font fontName="SansSerif" size="14" isBold="true"/>
				</textElement>
				<textFieldExpression><![CDATA["Note:  " + $F{note}]]></textFieldExpression>
			</textField>
		</band>
	</detail>
	<columnFooter>
		<band height="45">
			<property name="com.jaspersoft.studio.unit.height" value="px"/>
		</band>
	</columnFooter>
</jasperReport>'as xml), jasper=NULL
WHERE id_report=2;