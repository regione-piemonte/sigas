...
public interface SigasValidazioneRepository extends CrudRepository<SigasValidazione, Long> {
	...
	@Transactional
	@Modifying
	@Query(value="INSERT INTO sigas_validazione (id_valid, codice_azienda, anno, stato) VALUES(nextval('SEQ_ALIQUOTE'), ?1, ?2, ?3) ON CONFLICT ON CONSTRAINT ak_validazione_02 DO NOTHING ", nativeQuery=true)
