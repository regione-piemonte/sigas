...
public class XSRFProtectionFilter implements Filter {
			...
			if (!isRestRequest(hreq)) {
				...
				if (isXSRFSessionAlive(session)) {
					...
					if (validXSRFCookieAndHeader(hreq, session)) {
						...
						hresp.getWriter().write(createResponseSessionInvalid());
						return;
					}
				} else {
	...
	private String createResponseSessionInvalid() throws JsonGenerationException, JsonMappingException, IOException {
		ExceptionVO ex = new ExceptionVO(HttpStatus.FORBIDDEN.value(), "La sessione non è più valida si prega di rieffettuare il login", ErrorCodes.SESSIONE_NON_VALIDA);
		ResponseVO<ExceptionVO> resp = new ResponseVO<ExceptionVO>(Esito.ERROR, ex);
		ObjectMapper objectMapper = new ObjectMapper();
		return objectMapper.writeValueAsString(resp);
	}

	/**
	 * E' necessario fare in modo che le risorse (pagine, etc) non siano
	 * mantenute nella cache del browser per evitare che, ad un accesso in una
	 * sessione successiva, la fase di ingresso (e quindi di ricreazione del
	 * token) venga bypassata. Se cio' accadesse le richieste REST sarebbero le
	 * prime ad arrivare al server e si ricadrebbe nella casistica di token non
	...
	private boolean validXSRFCookieAndHeader(HttpServletRequest hreq, HttpSession session) {
		...
		return actualRequestHeader != null && actualActiveToken != null && actualActiveToken.equals(actualRequestHeader) && actualActiveToken.equals(actualRequestCookie);
	}
