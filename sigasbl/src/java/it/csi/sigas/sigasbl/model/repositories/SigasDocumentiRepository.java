...
public interface SigasDocumentiRepository extends CrudRepository<SigasDocumenti, Integer> {
	...
	@Query(value=
			...
			" where " + 
					...
					"(:idTipoDocumento = 0 " +
						 " OR sd.id_tipo_documento in (select id_tipo_documento " + 
					  						  	  	 "from sigas_tipo_documento " +
					  						  	  	 "where id_tipo_documento_padre = :idTipoDocumento) " + 
					  	 " OR sd.id_tipo_documento = :idTipoDocumento) and " + 
				  	"(:insUser='' OR sd.ins_user= :insUser) and " +
				  	"sd.id_stato != 5 " +
			" UNION ALL " +
			" select sd_ext.* " + 
			" from sigas_documenti sd_ext " + 
			" where sd_ext.id_stato = 5 " +
				  " and sd_ext.rif_archivio in ( " +
												"select sd.n_protocollo " + 
												" from sigas_documenti sd " + 
												" where " + 
														"(:idAnag=0 OR sd.id_anag = :idAnag) and " +
														"(:nProtocollo='' OR sd.rif_archivio = :nProtocollo OR sd.n_protocollo = :nProtocollo) and " +
														...
														"(:idTipoDocumento = 0 " +
															 " OR sd.id_tipo_documento in (select id_tipo_documento " + 
														  						  	  	 ...
														  						  	  	 "where id_tipo_documento_padre = :idTipoDocumento) " + 
														  	 " OR sd.id_tipo_documento = :idTipoDocumento) and " + 
													  	"(:insUser='' OR sd.ins_user= :insUser) " +											
											 ") ",nativeQuery = true)
			...
			" where " + 
					...
					"(:idTipoDocumento = 0 " +
						 " OR sd.id_tipo_documento in (select id_tipo_documento " + 
					  						  	  	 "from sigas_tipo_documento " +
					  						  	  	 "where id_tipo_documento_padre = :idTipoDocumento) " + 
					  	 " OR sd.id_tipo_documento = :idTipoDocumento) " + 
				  	//" and (:insUser='' OR sd.ins_user= :insUser) and " +
				  	" and sd.id_stato != 5 " +
			" UNION ALL " +
			" select sd_ext.* " + 
			" from sigas_documenti sd_ext " + 
												...
												" where " + 
														...
														"(:idTipoDocumento = 0 " +
															 " OR sd.id_tipo_documento in (select id_tipo_documento " + 
														  						  	  	 ...
														  						  	  	 "where id_tipo_documento_padre = :idTipoDocumento) " + 
														  	 " OR sd.id_tipo_documento = :idTipoDocumento) " + 
													  	//" and (:insUser='' OR sd.ins_user= :insUser) " +											
											 ") ",nativeQuery = true)
	List<SigasDocumenti> ricercaDocumenti(@Param("idAnag") Long idAnag, 
										  @Param("nProtocollo") String nProtocollo, 
				...
				" where " + 
						...
						"(:idTipoDocumento = 0 " +
							 " OR sd.id_tipo_documento in (select id_tipo_documento " + 
						  						  	  	 ...
						  						  	  	 "where id_tipo_documento_padre = :idTipoDocumento) " + 
						  	 " OR sd.id_tipo_documento = :idTipoDocumento) and " + 
					  	" (:insUserList IS NULL OR sd.ins_user in (:insUserList)) and " +
					  	"sd.id_stato != 5 " +
				" UNION ALL " +
				" select sd_ext.* " + 
				" from sigas_documenti sd_ext " + 
													...
													" where " + 
															...
															"(:idTipoDocumento = 0 " +
																 " OR sd.id_tipo_documento in (select id_tipo_documento " + 
															  						  	  	 ...
															  						  	  	 "where id_tipo_documento_padre = :idTipoDocumento) " + 
															  	 " OR sd.id_tipo_documento = :idTipoDocumento) and " + 
														  	" (:insUserList IS NULL OR sd.ins_user in (:insUserList)) " +
												
												 ") ",nativeQuery = true)
	List<SigasDocumenti> ricercaDocumentiPerLegaleRappresentante(@Param("idAnag") Long idAnag, @Param("nProtocollo") String nProtocollo, 
																  @Param("annualita") String  annualita, @Param("idTipoDocumento") Integer  idTipoDocumento,
																  @Param("insUserList") List<String>  insUserList);
